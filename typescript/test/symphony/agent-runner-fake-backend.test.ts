...
describe("AgentRunner with a synthetic backend", () => {
  ...
  test("aborted runs treat the backend port_exit 143 as cancellation", async () => {
    ...
    try {
      ...
      await turnStarted.promise;
      controller.abort(agentRunCancellation("issue_stalled", { session_id: "thread-stalled" }));
      releaseTurn();
      await runPromise;
  ...
  test("a run aborted before it starts never opens a session", async () => {
    ...
    const controller = new AbortController();
    controller.abort(agentRunCancellation("explicit_stop"));
