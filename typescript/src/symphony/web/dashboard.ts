...
function blockedActionCell(entry: Json): string {
  ...
  const identifier = str(entry.issue_identifier) || "this issue";
  const confirmMessage = `Rerun blocked agent for ${identifier}? This may execute tools or modify external tracker and repository state.`;
  return `<form method="post" action="${escapeAttr(endpoint)}" onsubmit="return confirm(${escapeAttr(JSON.stringify(confirmMessage))})"><button type="submit" class="subtle-button">Rerun</button></form>`;
}
